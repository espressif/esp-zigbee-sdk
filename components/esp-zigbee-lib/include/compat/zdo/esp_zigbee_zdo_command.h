...
#warning "This file is deprecated, please include "ezbee/zdo.h" instead!"
 ...
 *
 */
typedef void (*esp_zb_zdo_match_desc_callback_t)(esp_zb_zdp_status_t zdo_status,
                                                 uint16_t            addr,
                                                 uint8_t             endpoint,
                                                 void               *user_ctx);
 ...
 *
 */
typedef void (*esp_zb_zdo_ieee_addr_callback_t)(esp_zb_zdp_status_t         zdo_status,
 ...
 *
 */
typedef void (*esp_zb_zdo_node_desc_callback_t)(esp_zb_zdp_status_t    zdo_status,
                                                uint16_t               addr,
 ...
 *
 */
typedef void (*esp_zb_zdo_active_ep_callback_t)(esp_zb_zdp_status_t zdo_status,
                                                uint8_t             ep_count,
                                                uint8_t            *ep_id_list,
                                                void               *user_ctx);
 ...
 *
 */
typedef void (*esp_zb_zdo_simple_desc_callback_t)(esp_zb_zdp_status_t          zdo_status,
...
/**
 * @brief Zigbee ZDO bind destination address mode
 */
typedef enum {
    ESP_ZB_ZDO_BIND_DST_ADDR_MODE_16_BIT_GROUP    = EZB_ADDR_MODE_GROUP, /*!< 16-bit group address */
    ESP_ZB_ZDO_BIND_DST_ADDR_MODE_64_BIT_EXTENDED = EZB_ADDR_MODE_EXT,   /*!< 64-bit extended address */
} esp_zb_zdo_bind_dst_addr_mode_t;

/**
 * @brief The Zigbee ZDO bind command struct
 *
 * @note Current implementation of the API ONLY supports 64 bit extended address's address mode. Other address mode will support
 * later.
 *
 * @note Be aware of the one req_dst_addr is address that command send to, while dst_address is the destination of the binding
 * entry.
 *
...
typedef struct esp_zb_zdo_permit_joining_req_param_s {
    uint16_t dst_nwk_addr; /*!< NWK address that request sent to */
    uint8_t
        permit_duration; /*!< The length of time in seconds. 0x00 and 0xff indicate that permission is disabled or enabled */
    uint8_t tc_significance; /*!< If this is set to 0x01 and the remote device is the Trust Center, the command affects the
                              * Trust Center authentication policy as described in the sub-clauses below; If this is set to
                              * 0x00, there is no effect on the Trust Center */
...
typedef struct esp_zb_zdo_binding_table_record_s {
    ...
    esp_zb_addr_u
        dst_address; /*!< The destination address for the binding entry.16 or 64 bit. As specified by the dst_addr_mode field.*/
    uint8_t dst_endp; /*!< The destination endpoint for binding entry, this field shall be present only if the DstAddrMode field
                         has a value of 0x03 refer to esp_zb_zdo_bind_dst_addr_mode_t */
...
typedef struct esp_zb_network_descriptor_s {
    ...
    bool end_device_capacity; /*!< This value is set to true if the device is capable of accepting join requests from end
                                 devices and set to FALSE otherwise.*/
...
typedef struct esp_zb_zdo_mgmt_nwk_update_req_param_s {
    ...
    uint8_t  scan_duration;    /*!< A value used to calculate the length of time to spend scanning each channel. This field is
                                  within the range of 0x00 to 0x05 or 0xfe or 0xff.     If it has a value of 0xfe this is a request
                                  for     channel change.     If it has a value of 0xff this is a request to change the
                                  apsChannelMaskList and     nwkManagerAddr attributes. */
    ...
    uint16_t nwk_manager_addr; /*!< This field shall be present only if the scan_duration is set to 0xff, and, where present,
                                    indicates the NWK address for the device with the Network Manager bit set in its Node
                                  Descriptor.*/
    uint16_t dst_addr;         /*!< The destination network short address of request */
...
typedef struct esp_zb_zdo_mgmt_lqi_rsp_s {
    ...
    esp_zb_zdo_neighbor_table_list_record_t *neighbor_table_list; /*!< A list of descriptors, beginning with the StartIndex
                                                                     element and continuing for neighbor_table_list_count, of
                                                                     the elements in the Remote Device's Neighbor Table */
...
typedef struct esp_zb_zdo_mgmt_update_notify_s {
    ...
    uint32_t
        scanned_channels;           /*!< The (b27,...,b31) represent the binary encoded Channel Page, (b0, b1,... b26) indicate
    ...
    uint8_t  scanned_channels_list_count; /*!< The list shall contain the number of records contained in the EnergyValues
                                             parameter. */
 ...
 *
 */
typedef void (*esp_zb_zdo_scan_complete_callback_t)(esp_zb_zdp_status_t          zdo_status,
                                                    uint8_t                      count,
 ...
 *
 */
typedef void (*esp_zb_zdo_energy_detect_callback_t)(esp_zb_zdp_status_t                  status,
                                                    uint16_t                             count,
 ...
 *
 */
void esp_zb_zdo_find_on_off_light(esp_zb_zdo_match_desc_req_param_t *cmd_req,
 ...
 *
 */
void esp_zb_zdo_find_color_dimmable_light(esp_zb_zdo_match_desc_req_param_t *cmd_req,
                                          esp_zb_zdo_match_desc_callback_t   user_cb,
                                          void                              *user_ctx);
 ...
 */
esp_err_t esp_zb_zdo_match_cluster(esp_zb_zdo_match_desc_req_param_t *param,
                                   esp_zb_zdo_match_desc_callback_t   user_cb,
                                   void                              *user_ctx);
 ...
 * @param[in] user_cb  A user callback that will be called if received ieee address response refer to
 * esp_zb_zdo_ieee_addr_callback_t
 * @param[in] user_ctx A void pointer that contains the user defines additional information when callback trigger
 *
 */
void esp_zb_zdo_ieee_addr_req(esp_zb_zdo_ieee_addr_req_param_t *cmd_req,
                              esp_zb_zdo_ieee_addr_callback_t   user_cb,
                              void                             *user_ctx);
 ...
 * @param[in] user_cb  A user callback that will be called if received nwk address response refer to
 * esp_zb_zdo_nwk_addr_callback_t
 * @param[in] user_ctx A void pointer that contains the user defines additional information when callback trigger
 *
 */
 ...
 * @param[in] user_cb  A user callback that will be called if received node desc response refer to
 * esp_zb_zdo_node_desc_callback_t
 * @param[in] user_ctx A void pointer that contains the user defines additional information when callback trigger
 *
 */
void esp_zb_zdo_node_desc_req(esp_zb_zdo_node_desc_req_param_t *cmd_req,
                              esp_zb_zdo_node_desc_callback_t   user_cb,
                              void                             *user_ctx);
 ...
 * @param[in] user_cb  A user callback that will be called if received power desc response refer to
 * esp_zb_zdo_power_desc_callback_t
 * @param[in] user_ctx A void pointer that contains the user defines additional information when callback trigger
 *
 */
void esp_zb_zdo_power_desc_req(esp_zb_zdo_power_desc_req_param_t *cmd_req,
                               esp_zb_zdo_power_desc_callback_t   user_cb,
                               void                              *user_ctx);
 ...
 * @param[in] user_cb  A user callback that will be called if received simple desc response refer to
 * esp_zb_zdo_simple_desc_callback_t
 * @param[in] user_ctx A void pointer that contains the user defines additional information when callback trigger
 *
 */
void esp_zb_zdo_simple_desc_req(esp_zb_zdo_simple_desc_req_param_t *cmd_req,
                                esp_zb_zdo_simple_desc_callback_t   user_cb,
                                void                               *user_ctx);
 ...
 * @param[in] user_cb  A user callback that will be called if received active ep response refer to
 * esp_zb_zdo_active_ep_callback_t
 * @param[in] user_ctx A void pointer that contains the user defines additional information when callback trigger
 *
 */
void esp_zb_zdo_active_ep_req(esp_zb_zdo_active_ep_req_param_t *cmd_req,
                              esp_zb_zdo_active_ep_callback_t   user_cb,
                              void                             *user_ctx);
...
void esp_zb_zdo_device_leave_req(esp_zb_zdo_mgmt_leave_req_param_t *cmd_req,
                                 esp_zb_zdo_leave_callback_t        user_cb,
                                 void                              *user_ctx);
 ...
 * @param[in] user_cb  A user callback that will be called if received permit join response refer to
 * esp_zb_zdo_permit_join_callback_t
 * @param[in] user_ctx A void pointer that contains the user defines additional information when callback trigger
 *
 */
void esp_zb_zdo_permit_joining_req(esp_zb_zdo_permit_joining_req_param_t *cmd_req,
                                   esp_zb_zdo_permit_join_callback_t      user_cb,
                                   void                                  *user_ctx);
 ...
 * @param[in] user_cb  A user callback that will be called if received binding table response refer to
 * esp_zb_zdo_binding_table_callback_t
 * @param[in] user_ctx A void pointer that contains the user defines additional information when callback trigger
 *
 */
void esp_zb_zdo_binding_table_req(esp_zb_zdo_mgmt_bind_param_t       *cmd_req,
                                  esp_zb_zdo_binding_table_callback_t user_cb,
                                  void                               *user_ctx);
 ...
 * @param[in] user_cb A user callback that will be called if received Mgmt_Lqi_req response refer to
 * esp_zb_zdo_mgmt_lqi_rsp_callback_t
 * @param[in] user_ctx A void pointer that contains the user defines additional information when user_cb is triggered
 */
void esp_zb_zdo_mgmt_lqi_req(esp_zb_zdo_mgmt_lqi_req_param_t   *cmd_req,
                             esp_zb_zdo_mgmt_lqi_rsp_callback_t user_cb,
                             void                              *user_ctx);

/**
 * @brief Send ZDO Mgmt_NWK_Update_req command
 *
 * If the request is broadcast with a scan_duration of 0xFE or 0xFF, no response will be received. As a result, the user_cb will
 * be triggered with ESP_ZB_ZDP_STATUS_TIMEOUT after ESP_ZB_NWK_UPDATE_REQ_TIMEOUT seconds.
 *
 * @param[in] cmd_req A pointer indicates the fields of the Mgmt_NWK_Update_req command, @ref
 * esp_zb_zdo_mgmt_nwk_update_req_param_s
 * @param[in] user_cb  A user callback that will be called if received Mgmt_NWK_Update_notify refer to
 * esp_zb_zdo_binding_table_callback_t
 * @param[in] user_ctx A void pointer that contains the user defines additional information when callback trigger
 */
void esp_zb_zdo_mgmt_nwk_update_req(esp_zb_zdo_mgmt_nwk_update_req_param_t      *cmd_req,
                                    esp_zb_zdo_mgmt_nwk_update_notify_callback_t user_cb,
                                    void                                        *user_ctx);
...
#else
#error "This file is for compatibility, please include "ezbee/zdo.h"!"
#endif
