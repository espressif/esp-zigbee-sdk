...
typedef struct ezb_zdp_bind_req_field_s {
    ...
    ezb_addr_t dst_addr; /*!< Address of the destination device. The format of this field is determined by the value of
                            DstAddrMode. */
    uint8_t dst_ep;      /*!< Endpoint of the destination device. This field is present only if DstAddrMode is 0x03. */
...
typedef struct ezb_zdo_bind_req_s {
    ...
    ezb_zdo_bind_req_callback_t cb;           /*!< User callback for the result of ZDO Bind Request */
...
/**
 * @brief Send a ZDO Bind_req command.
 *
 ...
 *
 * @param[in] req The pointer to the ZDO bind request. See @ref ezb_zdo_bind_req_t.
 *
 * @return
...
/**
 * @brief Send a ZDO Unbind_req command.
 *
 ...
 *
 * @param[in] req The pointer to the ZDO unbind request. See @ref ezb_zdo_unbind_req_t.
 *
 * @return
 *      - EZB_ERR_NONE: Request sent successfully
 *      - Other error codes: See @ref ezb_err_t
