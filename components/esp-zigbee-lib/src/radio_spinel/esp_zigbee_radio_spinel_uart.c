...
static esp_err_t radio_spinel_uart_deinit(const esp_radio_spinel_uart_config_t *config)
{
...
static esp_err_t radio_spinel_uart_init(const esp_radio_spinel_uart_config_t *config,
                                        esp_line_endings_t tx_mode, esp_line_endings_t rx_mode)
{
    esp_err_t ret = ESP_OK;

    ESP_GOTO_ON_ERROR(radio_spinel_uart_init_port(config), exit, TAG, "Failed to initialize uart port");
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 3, 0)
...
static esp_err_t radio_spinel_init_uart(const esp_radio_spinel_uart_config_t *uart_config, int *uart_fd)
{
    char uart_path[16];
    ESP_RETURN_ON_ERROR(radio_spinel_uart_init(uart_config, ESP_LINE_ENDINGS_LF, ESP_LINE_ENDINGS_LF), TAG,
...
static esp_err_t radio_spinel_deinit_uart(const esp_radio_spinel_uart_config_t *uart_config, int *uart_fd)
{
    if (*uart_fd != -1) {
        close(*uart_fd);
        *uart_fd = -1;
        return radio_spinel_uart_deinit(uart_config);
    } else {
