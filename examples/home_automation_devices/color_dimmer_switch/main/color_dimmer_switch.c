 ...
 *
 * Zigbee HA_color_dimmer_switch Example
 *
...
#include "color_dimmer_switch.h"

static const char *TAG = "COLOR_DIMMER_SWITCH";
...
static void button_event_handler(switch_driver_handle_t handle)
{
    ...
    static bool     set_level  = true;
    static uint8_t  level      = 0;
    static uint16_t color_x    = 0;
    static uint16_t color_y    = 0;

    if (set_level) {
        ezb_zcl_level_move_to_level_with_on_off_cmd_t cmd_req = {
            .cmd_ctrl =
                {
                    .dst_addr.addr_mode = EZB_ADDR_MODE_NONE,
                    .src_ep             = ESP_ZIGBEE_HA_COLOR_DIMMER_SWITCH_EP_ID,
                },
            .payload =
                {
                    .level           = (level += 32),
        ...
        ezb_zcl_level_move_to_level_with_on_off_cmd_req(&cmd_req);
        esp_zigbee_lock_release();
        ESP_EARLY_LOGI(TAG, "Move the level of HA dimmable light to %d", cmd_req.payload.level);
        set_level = false;
    } else {
        ezb_zcl_color_control_move_to_color_cmd_t cmd_req = {
            ...
            .payload =
                {
                    .color_x         = (color_x += (1024)),
                    .color_y         = (color_y += (1024)),
                    .transition_time = 10, // 1 second
                },
        };
        esp_zigbee_lock_acquire(portMAX_DELAY);
        ...
        esp_zigbee_lock_release();
        ESP_EARLY_LOGI(TAG, "Move the color of HA dimmable light color_x: 0x%04x, color_y: 0x%04x", cmd_req.payload.color_x,
                       cmd_req.payload.color_y);
        set_level = true;
    }
}
...
static void zdo_bind_ha_light_device_result(const ezb_zdp_bind_req_result_t *result, void *user_ctx)
{
    ...
    if (result->error == EZB_ERR_NONE) {
        if (result->rsp && result->rsp->status == EZB_ZDP_STATUS_SUCCESS) {
            ESP_LOGI(TAG, "Bound HA color dimmable light device successfully");
        } else {
            ESP_LOGE(TAG, "Failed to bind HA color dimmable light device with status (0x%02x)", result->rsp->status);
        }
    } else {
        ESP_LOGE(TAG, "Failed to bind HA color dimmable light device with error (0x%04x)", result->error);
    }
}

static ezb_err_t zdo_bind_ha_color_dimmable_light_device(uint16_t dst_short_addr, uint8_t dst_ep)
{
    ...
    bind_req.cb               = zdo_bind_ha_light_device_result;
    ret                       = ezb_zdo_bind_req(&bind_req);
    if (ret == EZB_ERR_NONE) {
        ESP_LOGI(TAG, "Attempt to bind HA color dimmable light device (short address: 0x%04hx)", dst_short_addr);
    } else {
        ESP_LOGE(TAG, "Failed to bind HA color dimmable light device (short address: 0x%04hx) with error(0x%04x)",
                 dst_short_addr, ret);
    }
    return ret;
}

static void zdo_find_ha_color_dimmable_light_device_result(const ezb_zdo_match_desc_req_result_t *result, void *user_ctx)
{
    assert(result);
    if (result->error == EZB_ERR_NONE) {
        if (result->rsp && result->rsp->status == EZB_ZDP_STATUS_SUCCESS && result->rsp->match_length > 0 &&
            result->rsp->match_list) {
            zdo_bind_ha_color_dimmable_light_device(result->rsp->nwk_addr_of_interest, result->rsp->match_list[0]);
        }
    } else {
        ESP_LOGE(TAG, "Failed to find HA color dimmable light device in the network with error(0x%04x)", result->error);
    }
}
...
esp_err_t esp_zigbee_create_zha_color_dimmer_switch_device(void)
{
...
static void esp_zigbee_stack_main_task(void *pvParameters)
{
    ...
    ESP_ERROR_CHECK(esp_zigbee_create_zha_color_dimmer_switch_device());
