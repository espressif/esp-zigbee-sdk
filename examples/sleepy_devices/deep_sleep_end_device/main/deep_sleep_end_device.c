 ...
 *
 * Zigbee Deep Sleep End Device Example
 *
...
#include "deep_sleep_end_device.h"

static const char *TAG = "DEEP_SLEEP_END_DEVICE";

#define DEEP_SLEEP_TIME_SLEEP_SEC  20 // In seconds
#define DEEP_SLEEP_TIME_WAKEUP_SEC 5  // In seconds
...
static esp_err_t esp_deep_sleep_weakup_config(void)
{
    ...
    ESP_RETURN_ON_ERROR(esp_sleep_enable_ext1_wakeup(BIT(CONFIG_GPIO_EXT1_WAKEUP_SOURCE), ESP_EXT1_WAKEUP_ANY_LOW), TAG,
                        "Failed to enable ext1 wakeup");
...
{
    if (result && result->error == EZB_ERR_NONE) {
