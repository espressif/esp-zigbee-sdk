 ...
 *
 * Zigbee Touchlink Initiator Example
 *
...
#include "touchlink_initiator.h"

static const char *TAG = "TOUCHLINK_INITIATOR";
...
static void button_event_handler(switch_driver_handle_t handle)
{
    ...
    if (ezb_bdb_dev_joined()) {
        ...
        esp_zigbee_lock_release();
        ESP_EARLY_LOGI(TAG, "Scanning as a Touchlink initiator...");
    }
}
...
static esp_err_t zdo_bind_ha_light_device(uint16_t dst_short_addr, uint8_t dst_ep)
{
...
static esp_err_t zdo_find_ha_light_device(void)
{
    esp_err_t ret             = ESP_FAIL;
...
static bool esp_zigbee_app_signal_handler(const ezb_app_signal_t *app_signal)
{
    ...
    case EZB_BDB_SIGNAL_TOUCHLINK_INITIATOR_FINISHED: {
        ...
        if (status == EZB_BDB_STATUS_SUCCESS) {
            ...
            ESP_LOGI(TAG, "Commissioning successfully: PAN ID(0x%04hx, EXT: 0x%llx), Channel(%d), Short Address(0x%04hx)",
                     ezb_nwk_get_panid(), extended_pan_id.u64, ezb_nwk_get_current_channel(), ezb_nwk_get_short_address());
            zdo_find_ha_light_device();
            ESP_LOGI(TAG, "Touchlink commissioning successfully");
        } else {
            ESP_LOGW(TAG, "No Touchlink target devices found with status(0x%02x)", status);
            uint8_t network_key[16];
            ezb_secur_get_network_key(network_key);
            ESP_LOG_BUFFER_HEX("Network key", network_key, 16);
...
esp_err_t esp_zigbee_setup_commissioning(void)
{
    ...
    ESP_ERROR_CHECK(ezb_touchlink_set_channel(ESP_ZIGBEE_TOUCHLINK_DEVICE_CHANNEL));
