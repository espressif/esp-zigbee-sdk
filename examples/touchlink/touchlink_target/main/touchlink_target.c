...
static const char *TAG = "TOUCHLINK_TARGET";
...
static bool esp_zigbee_app_signal_handler(const ezb_app_signal_t *app_signal)
{
    ...
    case EZB_BDB_SIGNAL_DEVICE_REBOOT: {
        ...
        if (status == EZB_BDB_STATUS_SUCCESS) {
            ...
            if (ezb_bdb_is_factory_new()) {
                ezb_bdb_start_top_level_commissioning(EZB_BDB_MODE_TOUCHLINK_TARGET);
            } else {
    ...
    case EZB_BDB_SIGNAL_TOUCHLINK_TARGET_FINISHED: {
        ...
        if (status == EZB_BDB_STATUS_SUCCESS) {
            ...
            ESP_LOGW(TAG, "Touchlink commissioning failed with status(0x%02x)", status);
        }
        uint8_t network_key[16];
        ezb_secur_get_network_key(network_key);
        ESP_LOG_BUFFER_HEX("Network key", network_key, 16);
        ESP_LOGI(TAG, "Touchlink target finished with status(0x%02x)", status);
    } break;
